...
1. **Mount the $\beta$-dial on the Hou-Luo geometry** (BUILDER + ADVERSARY, the lead): the $\beta$-dial is DONE on Taylor-Green (`vortex_stretching/beta_dial.py`, LEARNINGS #14: laminar flow sustains $\alpha \ge 1$, staircase not descending). Combine with the queued $\nu$-sweep: track the local coherence exponent $\alpha$ and the peak amplification on `experiments/hou_luo/` as $\nu$ decreases at fixed data, with per-$\nu$ refinement convergence (LEARNINGS #11/#13 discipline). The readout: does sustained coherence degrade as intensity grows, and does amplification diverge or saturate toward the Euler limit?
2. **Lean: state the energy inequality cleanly** (VERIFIER): attempt a Mathlib-faithful statement of $\tfrac12\|u(t)\|_2^2 + \nu\int_0^t\|\nabla u\|_2^2 \le \tfrac12\|u_0\|_2^2$, identifying the exact Mathlib gap (vector-valued Sobolev spaces). Document the gap as a VERIFIER target. Secondary Lean rung suggested by #14: the $\beta$-ladder scaling fact $[\xi_\lambda]_{C^\beta} = \lambda^\beta [\xi]_{C^\beta}$.
3. **Discrepancy adjudication** (ADVERSARY, small): dossier section 12 items 3-4 (Kukavica-Ziane 2006 vs 2007 distinction in `docs/02_graduate/regularity_criteria.md`; currency-dependence wording in `docs/02_graduate/scaling_and_supercriticality.md`). Items 1-2 already applied to LEARNINGS #2/#12.
4. **What forces depletion?** (open question sharpened by #14): depletion $\approx 0.55$ is present without sparseness in laminar flow, so the two geometric mechanisms are empirically split. A SURVEYOR pass on depletion mechanisms (alignment dynamics vs level-set geometry) would feed Direction 02.
5. **Octave ledger post-processor** (BUILDER, the cross-field forge's funded-first probe, cheap): `experiments/octave_ledger/` running on the STORED Hou-Luo and vortex-stretching outputs; measures the per-octave toll ratio $q$, the gear ratio $g$, and depletion at peak, with the circulation-morphology diagnostics co-mounted ($\Gamma_{act}/\nu$, sheet-vs-tube aspect, radial $\Gamma(s)$). Natural to fold into action 1's instrumentation.
6. **Intensity-conditioned (H)** (BUILDER analytic target from the forge): the repaired form of the weighted Harnack inequality for $\int(\omega\cdot S\omega)G^u$, conditioned on a scale-invariant intensity hypothesis so the long-time mixing degeneration (the audit's counterexample direction) is excluded. The exact identity and the critical $W$ are in `docs/03_research/forge_crossfield/`. Secondary VERIFIER rung: the halo lemma (elementary, Lean-shaped).
...
- Environment note: matplotlib is binary-incompatible with the installed NumPy 2.4.2, so `beta_dial.py` skips its $\theta(h)$ plot (data is in the cache). Fix or pin when plots are next needed.
...
| 2026-06-11 | Two parallel agent deliverables: Arch 2 dossier + Arch 4 Hou-Luo experiment | 1f6105d, afc2884 | `docs/research_atlas/conditional_criteria_dossier.md` (master table, log-thin frontier, CF $\beta$-ladder with open $\beta = 0$ endpoint; LEARNINGS #12); `experiments/hou_luo/` (axisymmetric NS with swirl, BKM converged 64/128/256^2, no-swirl and $\nu/4$ controls; LEARNINGS #13). PLAN rows (h), (i); LEARNINGS #2 refined per discrepancy log. |
| 2026-06-11 (cont.) | The $\beta$-dial + sparseness probe (dossier handoffs 11.3.1-2) | pending | `vortex_stretching/beta_dial.py` (LEARNINGS #14): seminorms bounded at all rungs, local exponent $\alpha \approx 1.2$ to $1.4$ (CF Lipschitz rung sustained); 2D control exactly zero through the same code path; sparseness clean negative, so depletion is not sparseness-driven in laminar flow. Independently re-run, numbers reproduced. |
| 2026-06-11 (cont. 2) | Cross-field conjecture forge (full protocol: 3 forges, adversary gate pass with identity audits, precedent checks) | pending | `docs/03_research/forge_crossfield/` (LEARNINGS #15): six cards, five wounded, one dead, none unconditionally alive. Keepers: the exact backward-kernel vorticity identity (audit-confirmed; critical $W$; everything reduced to intensity-conditioned (H)) and the halo lemma ($\tfrac{\sqrt3}{4}\Gamma^2 L \log(L/a)$, audit-confirmed). Three foreign lenses converged independently on pressure-plus-depletion, corroborating Direction 02. Funded-first probe: octave ledger post-processor. |
